<template>
	<view class="content">
		<view class="navbar" :style="{position:headerPosition,top:headerTop}">
			...
			<span class="iconfont yticon cate-item " @click="toggleCateMask('show')">&#xe6ce;</span>
		</view>
	...
	export default {
		...
		methods: {
			...
			async loadData(type='add', loading){
				...
				let kwd = this.kwd;
				//let t = this.t;
				let by = this.filterIndex == 0 ? 'general_index' : (this.filterIndex == 1 ? 'itemsale':'itemprice');
				let desc = 'desc';
				...
				if(kwd){
					this.$api.msg('正在为您搜索：'+kwd);
				}
				let goodsList = await this.$api.request('/product/lists', 'GET', {fid:this.fId,sid:this.sId,page:this.page,by:by,desc:desc,kwd:kwd});
