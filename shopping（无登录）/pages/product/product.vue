<template>
	<view class="container">
		...
		<view class="c-list">
			...
			<view class="c-row b-b" v-if="product.use_spec" @click="address">
				<text class="tit">发货地址</text>
				<!-- #ifdef MP -->
				<text>{{addressData[0].province.name}}</text>
				<text>{{addressData[0].city.name}}</text>
				<text>{{addressData[0].area.name}}</text>
				<!-- #endif -->
			...
			<!-- <view @click="address" class="configuration">
				<text>配送至：</text>
				<view>
					<text>{{addressData[0].province.name}}</text>
					<text>{{addressData[0].city.name}}</text>
					<text>{{addressData[0].area.name}}</text>
					<text class="selected-text" v-if="addressData.province.name+addressData.city.name+addressData.area.name">
						{{addressData.province.name+addressData.city.name+addressData.area.name}}
					</text>
				</view>
			</view>
			...
			<!-- <view class="c-row b-b" @click="toggleCoupon" v-if="product.coupon">
...
-->
		...
		<view class="page-bottom">
			<navigator url="/pages/index/index" open-type="switchTab" class="p-b-btn">
				...
				<text class="wenzi">首页</text>
			</navigator>
			<!-- #ifdef  MP -->
			<view class="serviced">
				<button open-type="contact" class="service iconfont yticon ">
					&#xe665;
				</button>
				<text class="wenzi">客服</text>
			</view>
			<!-- #endif -->
			<!-- #ifdef  H5 -->
			<view open-type="switchTab" class="p-b-btn" @click="service">
				<span class="iconfont yticon">&#xe665;</span>
				<text class="wenzi">客服</text>
			</view>
			<!-- #endif -->
			...
			<view class="action-btn-group">
				<button v-if="!flash" type="primary" class=" action-btn no-border add-cart-btn" @click="addCollect">加入收藏</button>
		...
		<view class="popup spec" :class="specClass" @touchmove.stop.prevent="stopPrevent" @click="toggleSpec">
			...
			<view class="layer attr-content" @click.stop="stopPrevent">
				<view class="a-t">
					<image v-if="specProduct.image" mode="aspectFill" :src="specProduct.image" @click="previewImageSku(specProduct.image)"></image>
					<view class="right">
						<text class="price">¥{{specProduct.sales_price}}</text>
...
<script>
	...
	export default {
		...
		computed: {
			...
			// 		console.log( this.addressData.province.name+this.addressData.city.name+this.addressData.area.name);
			// 		return this.addressData.province.name+this.addressData.city.name+this.addressData.area.name;
			// 	},
			specProduct() {
		...
		methods: {
			...
			service() {
				...
				var c = "&id=" + this.userInfo.id + "&c1=" + this.userInfo.mobile + "&c2=" + this.flash + "&c3=" + this.userInfo.avatar +
					"&c4=" + this.userInfo.openid + "&c5=" + this.flash;
				window.location =
					"https://yzf.qq.com/xv/web/static/chat/index.html?sign=37ef9b97d52301977346c8ec1ee2b76a3e61d341204a6655ca3332f2c8347d230ae7543785729c6be6b888f9772e0d207fea26c6" +
					c
			},
			// 为0时刷新页面
			timeup() {
			...
			async getDetail(id, flash_id) {
				...
				// 有问题
				// product是商品详情那个商品，但是将对象变成数组，他就空
				var newArr = [];
				// console.log(newArr);
			...
			// 发货地址
			async addList() {
				this.addressData = await this.$api.request('/address/all');
			...
			async buy() {
				...
				if (is_login) {
					//this.$api.msg('秒杀商品不能收藏');
					if (this.specProduct.stock < 1) {
						this.$api.msg('库存不足，无法购买');
						return;
					}
					...
					let url = `/pages/order/createOrder?id=${this.product.product_id}&spec=${spec}&skuid=${this.product.code}`;
					if (this.flash) {
			...
			// 		console.log("this.specSelected");
			// 		console.log(this.specSelected);
			// 		let data = await this.$api.request('/cart/add?id=' + this.product.product_id + '&spec=' + encodeURIComponent(spec));
			// 		if (data) {
			...
			// 查看sku图片
			previewImageSku(image) {
				//this.$api.msg('点击成功'+image);
				uni.previewImage({
					urls: [image],
				})
			},
